	...
	<form method="dialog">
		...
		<div class="container">
			...
			<label>
				...
				<div class="password">
					<input type="password" id="apiKey" /> <button id="showHidePassword">Show</button>
				</div>
...
`;
  ...
  const showHidePassword = dialog.querySelector("#showHidePassword");
  showHidePassword.addEventListener("click", function (e) {
    if (showHidePassword.innerHTML === "Show") {
    ...
    console.log(showHidePassword.innerHTML);
  });
...
async function generateMap(selection) {
  ...
  if (response.which === ButtonsEnum.CANCEL) {
    // Dialog cancelled
    return;
  }
  ...
  if (inputValues.apiKey == "") {
    error(
      "Error",
      'Missing Google Static Maps API key. <a href="https://developers.google.com/maps/documentation/maps-static/get-api-key">Generate one!</a>'
    );
    return;
  }
  ...
  for (let node of selection.items) {
    ...
    // Save settings
    try {
      await utils.storageHelper.set(
        "apiKey",
        dialog.querySelector("#apiKey").value
      );
    } catch (errMsg) {
    ...
    try {
      await utils.storageHelper.set(
        "styles",
        dialog.querySelector("#styles").value
      );
    } catch (errMsg) {
      await error("Error", errMsg);
      return;
    ...
    const url =
      ...
      "center=" +
      encodeURIComponent(inputValues.location) +
      "&zoom=" +
      encodeURIComponent(inputValues.zoom) +
      "&size=" +
      ...
      "&scale=2" +
      "&maptype=" +
      encodeURIComponent(inputValues.mapType) +
      (inputValues.locationPin
        ? "&markers=color:red%7C" + encodeURIComponent(inputValues.location)
        : "") +
      mapStyles +
      "&key=" +
      encodeURIComponent(inputValues.apiKey);
  ...
  if (finishMsg) {
    // Show done dialog only if some layers are skipped
